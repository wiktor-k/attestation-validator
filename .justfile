...
deps:
    ...
    mkdir -p target
    cargo deny "${ARGS[@]}" check 2>&1 | tee target/deny
...
test:
    ...
    # doctests are not currently run by cargo-nextest
    # CHECK https://github.com/nextest-rs/nextest/issues/16
...
generate kind:
    cargo xtask generate {{ kind }}

# Test before publishing the package
[group('ci')]
[metadata('gitlabci-job', '{"stage":"deploy","rules":[{"if":"$CI_MERGE_REQUEST_TITLE =~ /^Release/"}]}')]
[metadata('pacman', 'cargo-edit', 'cargo-semver-checks', 'rust')]
pre-publish:
    #!/usr/bin/bash
    set -euxo pipefail
    ...
    # if deps have been upgraded publish will fail because of dirty workspace
    cargo publish --dry-run

# Publish the package
[group('ci')]
[metadata('github-if', "startsWith(github.ref, 'refs/tags/v')")]
[metadata('gitlabci-job', '{"stage":"deploy","rules":[{"if":"$CI_COMMIT_TAG =~ /^v\\d/"}]}')]
[metadata('pacman', 'cargo-edit', 'rust')]
publish:
    #!/usr/bin/bash
    set -euxo pipefail

    if [[ -z "${CI_COMMIT_TAG:-}" ]]; then
        printf "The job runs only when \$CI_COMMIT_TAG environment variable is set.\n" >&2
        exit 1
    fi

    tag_version="${CI_COMMIT_TAG/v/}"

    # set the version in Cargo.toml to the same one as the tag
    cargo --locked set-version --package wkd-exporter "$tag_version"

    # if the version does not match cargo publish will fail because of dirty working directory
    cargo publish
